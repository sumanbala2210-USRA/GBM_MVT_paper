...
def GBM_det_analysis(input_info: dict, output_info: dict):
    ...
    for i in range(NN_analysis):
        ...
        all_runs_results.append(optimal_dets)
        all_combined_snr_list.extend(all_det_snr)
        single_det_snr_list.append({'iteration': i + 1, 'detector_snrs': single_det_snrs})
        #print(f"  -> Optimal list for this run: {optimal_dets} (SNR: {max_snr:.2f})")

        if i == 0:
            snr_order_combination_list = [entry['dets'] for entry in all_det_snr]
            print(f"  -> Detector SNR ranking (run 1): {snr_order_combination_list}")
            #print(f"  -> Simple Detector SNRs (run 1): {all_det_snr['dets']}")

    # Save the detailed results of the first run for reference
    detailed_df = pd.DataFrame(all_combined_snr_list)
    detailed_csv_path = output_info['file_path'] / f"Detailed_Det_Optimization_{output_info['file_info']}.csv"
    detailed_df.to_csv(detailed_csv_path, index=False)
    print(f"Detailed SINGLE detector SNR saved to \n{detailed_csv_path}")

    # --- Restructure the detector SNR data into a wide format ---
    # This creates a list where each item is a dictionary representing one row (one iteration)
    records_for_df = []
    for run_result in single_det_snr_list:
        # Start the row with the iteration number
        row_data = {'iteration': run_result['iteration']}
        # Create a dictionary of {detector_name: snr} for this run
        snr_map = {item['det']: item['snr'] for item in run_result['detector_snrs']}
        # Merge the two dictionaries
        row_data.update(snr_map)
        records_for_df.append(row_data)

    # Create the DataFrame from the restructured list of records
    all_det_snrs_df = pd.DataFrame(records_for_df)

    # --- Ensure columns are in a logical order (iteration, n0, n1, ...) ---
    column_order = ['iteration'] + dets
    # Reorder the DataFrame, handling cases where a detector might be missing for some reason
    all_det_snrs_df = all_det_snrs_df.reindex(columns=column_order)

    # --- Save the new, wide-format DataFrame to CSV ---
    all_det_snrs_csv_path = output_info['file_path'] / f"All_Det_SNRs_{output_info['file_info']}.csv"
    all_det_snrs_df.to_csv(all_det_snrs_csv_path, index=False, float_format='%.2f')
    print(f"All Combined SNR saved to \n{all_det_snrs_csv_path}")

    # --- 2. Calculate Mean SNR, Rank Detectors, and Create Final Combination List ---
    # Drop the iteration column to work only with SNR data
    snr_data = all_det_snrs_df.drop(columns=['iteration'])
    
    # Calculate the mean SNR for each detector across all runs.
    # You can easily switch this to .median() if you find it's more robust.
    mean_snrs = snr_data.mean()
    
    # Sort the detectors by their mean SNR to get the final ranked order
    final_ranked_detectors = mean_snrs.sort_values(ascending=False).index.tolist()
    
    # Create the cumulative list of combinations from the final ranked order
    ...
    # --- Print the final results you wanted ---
    print("\n----------------------------------------------------")
    print("--- Final Ranking Based on Mean SNR Across All Runs ---")
    print(f"\nFinal Detector Rank Order: {final_ranked_detectors}")
    print("\nFinal Cumulative Combinations List:")
    
    # The final list you asked for, now stored in the 'cumulative_combinations' variable
    print(cumulative_combinations)
    print("----------------------------------------------------")
    
    # --- 3. Find the most common optimal list across all runs ---
    # We convert lists to tuples so they can be counted
    ...
    tuple_results = [tuple(sorted(res)) for res in all_runs_results]
    
    if not tuple_results:
        print("\nNo successful runs found to determine the best detector list.")
        return []
        
    most_common_tuple = Counter(tuple_results).most_common(1)[0][0]
    final_det_list = list(most_common_tuple)
    
    print("\n--- Analysis Complete ---")
    print(f"The most frequently optimal detector list is: {final_det_list}")

    return all_combined_snr_list
