...
from TTE_SIM_v2 import Function_MVT_analysis, print_nested_dict, check_param_consistency, flatten_dict, GBM_MVT_analysis_det, GBM_MVT_analysis_complex, Function_MVT_analysis_complex, GBM_det_analysis
...
def generate_analysis_tasks(config: Dict[str, Any]) -> 'Generator':
    ...
    bin_widths_to_analyze_ms = analysis_settings.get('snr_bw_ms', [16, 32, 64])
    detector_selections = ['all']  # analysis_settings.get('detector_selections')
...
def analyze_one_group(task_info: Dict, data_path: Path, results_path: Path) -> List[Dict]:
    ...
    if sim_type == 'gbm':
        ...
        snr_evolution_results = GBM_det_analysis(input_info=analysis_input,
                    output_info = { 'file_path': output_analysis_path,
                                'file_info': param_dir.name,
                                'selection_str': selection_str})

    else:
        print("This simulation type is not required !!!")
        final_summary_list = []
        logging.error(f"Unsupported simulation type: {sim_type}. Skipping analysis for {param_dir.name}.")
        exit(1)
...
def main(config_filepath: str):
    ...
    run_results_path = results_path / f"det_{e_n(analysis_bin)}_{now}"
