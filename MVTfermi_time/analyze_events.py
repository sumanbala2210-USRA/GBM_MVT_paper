    ...
    for campaign in config.get('simulation_campaigns', []):
        ...
        for pulse_config in pulses_to_run:
            ...
            if pulse_shape in complex_pulse_list:
                ...
                total_sim_analysis = campaign['constants'].get('total_sim', 30)
...
def analyze_one_group(task_info: Dict, data_path: Path, results_path: Path) -> List[Dict]:
    ...
    if sim_type == 'gbm':
        ...
        if base_params['pulse_shape'] in complex_pulse_list:
            if time_res:
                logging.error("Time-resolved analysis is not supported for 'gbm' simulations with complex pulses.")
                return []
            else:
        ...
        if base_params['pulse_shape'] in complex_pulse_list:
            if time_res:
                final_summary_list = Function_MVT_analysis_complex_time_resolved(input_info=analysis_input,
                    output_info={ 'file_path': output_analysis_path,
                                 'file_info': param_dir.name,
                                 'selection_str': selection_str})
            else:
            ...
            if time_res:
                final_summary_list = Function_MVT_analysis_time_resolved(input_info=analysis_input,
                    output_info={ 'file_path': output_analysis_path,
                                'file_info': param_dir.name,
                                'selection_str': selection_str})
            else:
...
def main(config_filepath: str):
    ...
    final_results_csv_path = run_results_path / final_file_name
