...
#csv_2 = '10_Complex_S_3_bw0p01_mvt.csv'
csv_3 = '10_C_S_3_01_real.csv'

output_png = '10_figure_master_grid_landscape.png'
output_pdf = '10_figure_master_grid_landscape.pdf'

# --- 2. Publication Style Configuration ---
plt.rcParams.update({
    'font.size': 22, 'axes.labelsize': 22, 'xtick.labelsize': 16,
    'ytick.labelsize': 18, 'legend.fontsize': 18, 'legend.title_fontsize': 20,
...
try:
    ...
    print(f"Error loading data: {e}. Make sure both CSV files are present.")
    exit()
...
fig, axes = plt.subplots(4, len(peak_amp_ratios), figsize=(28, 17), 
...
for col_idx, peak_val in enumerate(peak_amp_ratios):
    ...
    for i, pos in enumerate(positions_to_compare):
        for j, sig in enumerate(sigmas_to_compare):
            ...
            ax.errorbar(subset['overall_amplitude'], subset['median_mvt_ms'], yerr=[subset['mvt_err_lower_ms'], subset['mvt_err_upper_ms']],
    ...
    for i, sig in enumerate(sigmas_to_compare):
        for j, pos in enumerate(positions_to_compare):
            ...
            ax.errorbar(subset['overall_amplitude'], subset['median_mvt_ms'], yerr=[subset['mvt_err_lower_ms'], subset['mvt_err_upper_ms']],
                        ...
                        markersize=10, markeredgewidth=0.8)
            ax.text(0.05, 0.95, f'A{col_idx + 1}', transform=ax.transAxes, 
            fontsize=18, fontweight='bold', va='top', bbox=dict(boxstyle='round,pad=0.3', fc='white', ec='none', lw=0, alpha=0.7))
    ...
    for i, shape in enumerate(new_shapes_to_compare):
        for j, pos in enumerate(positions_to_compare):
            ...
            fontsize=18, fontweight='bold', va='top', bbox=dict(boxstyle='round,pad=0.3', fc='white', ec='none', lw=0, alpha=0.7, zorder=100))
...
all_row_info = [
    ...
    {'fixed': "sigma=10, bin=0.1ms", 'color_title': 'Pulse Shape', 'marker_title': 'Position (s)', 'color_labs': ['Long', 'Short'], 'marker_labs': positions_to_compare},
...
for row_idx in range(4): # Loop through all 5 rows
    try:
        axes[row_idx, 0].text(0.05, 0.05, f"Fixed:\n{all_row_info[row_idx]['fixed']}", transform=axes[row_idx, 0].transAxes, fontsize=20, va='bottom', bbox=dict(boxstyle='round,pad=0.3', fc='white', alpha=0.7))
    except:
        axes[row_idx, 0].text(0.05, 0.05, f"Fixed Pulse:\n{all_row_info[row_idx]['fixed pulse']}", transform=axes[row_idx, 0].transAxes, fontsize=20, va='bottom', bbox=dict(boxstyle='round,pad=0.3', fc='white', alpha=0.7))
...
fig.supxlabel('Overall Amplitude', fontsize=24, y=0.02)
